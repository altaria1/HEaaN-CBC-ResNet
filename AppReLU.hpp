...
void ApproxReLU6(HEaaN::Context context, HEaaN::HomEvaluator eval, HEaaN::Ciphertext& ctxt, HEaaN::Ciphertext& ctxt_relu) {
...
void ApproxReLU_bundle(Context context, HomEvaluator eval, vector<vector<Ciphertext>>& ctxt_bundle, vector<vector<Ciphertext>>& ctxt_relu){
    ...
    if (n1==16 && n2==16){
        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[i / 5][i % 5], ctxt_relu[i / 5][i % 5]);
        }

        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[8+(i /5)][(i%5)], ctxt_relu[8+(i /5)][(i%5)]);
        }

        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[i / 5][5+(i%5)], ctxt_relu[i / 5][5+(i%5)]);
        }

        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[8+(i /5)][5+(i%5)], ctxt_relu[8+(i /5)][5+(i%5)]);
        }

        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[i / 5][10+(i%5)], ctxt_relu[i / 5][10+(i%5)]);
        }

        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[8+(i /5)][10+(i%5)], ctxt_relu[8+(i /5)][10+(i%5)]);
        }

        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 16; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[i][15], ctxt_relu[i][15]);
        }
    }
    ...
    if (n1==4 && n2==32){
        ...
        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[i / 10][i % 10], ctxt_relu[i / 10][i % 10]);
        }

        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[i / 10][10+(i % 10)], ctxt_relu[i / 10][10+(i % 10)]);
        }

        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[i / 10][20+(i % 10)], ctxt_relu[i / 10][20+(i % 10)]);
        }
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            {
                ApproxReLU(context, eval, ctxt_bundle[i / 2][30+(i %2)], ctxt_relu[i / 2][30+ (i% 2)]);
            }
        }
    }
    ...
    if (n1==1 && n2==64){  
        ...
        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 40; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[0][i], ctxt_relu[0][i]);
        }
        #pragma omp parallel for num_threads(40)
        for (int i = 40; i < 64; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[0][i], ctxt_relu[0][i]);
    ...
    if (n1==4 && n2==16){  
        ...
        #pragma omp parallel for num_threads(40)
        for (int i = 0; i < 24; ++i) {
            ApproxReLU(context, eval, ctxt_bundle[i/6][10+(i%6)], ctxt_relu[i/6][10+(i%6)]);
        }
